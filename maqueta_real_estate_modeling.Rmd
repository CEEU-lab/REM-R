...
```{r}
pt_pts <- pt_data %>% st_as_sf(coords=c("lon","lat"), crs=4326)
json_char <- '{"type":"Polygon","coordinates":[[[-58.47894027107537,-34.65646579501466],[-58.47814027616029,-34.68541524020002],[-58.44314049862075,-34.68245496596179],[-58.444140492264154,-34.6536690060118],[-58.47894027107537,-34.65646579501466]]]}'
zone <- st_read(json_char)
zone_wgs <- st_transform(zone, 4326)
zone_points <- st_join(pt_pts, zone_wgs)
oferta <- na.omit(zone_points)


ggplot() + 
  #geom_sf(data=zone_points, alpha=0.9, color='red')+
  #geom_sf(data=pt_pts, alpha=0.9)+
  geom_sf(data=zone_wgs, size = 0.3,color="black")+
  geom_sf(data=barrios_sur_, alpha=0.7)
  geom_df(oferta, color="red")
```
